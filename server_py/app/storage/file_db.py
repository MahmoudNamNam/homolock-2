...
- JSON files (sessions.json, jobs.json) hold metadata and paths only; no raw ciphertexts/keys.
- Large blobs (params, keys, ciphertexts) are stored as files under blobs/.
...
# Lock file for DB operations (under base_dir)
DB_LOCK_NAME = ".db.lock"
SESSIONS_JSON = "sessions.json"
JOBS_JSON = "jobs.json"
DB_SUBDIR = "db"
BLOBS_SUBDIR = "blobs"
SESSIONS_BLOB_DIR = "sessions"
JOBS_BLOB_DIR = "jobs"
...
class FileDB:
    """
    File-based storage for sessions and jobs.
    - base_dir/db/sessions.json  -> { session_id: session_doc }
    - base_dir/db/jobs.json      -> { job_id: job_doc }
    - base_dir/blobs/sessions/<session_id>/  -> params.seal, public_key.seal, etc.
    - base_dir/blobs/jobs/<job_id>/         -> result.ct (and copied inputs for worker run)
    """

    def __init__(self, base_dir: str | Path):
        self.base = Path(base_dir)
        self.db_dir = self.base / DB_SUBDIR
        self.blobs_dir = self.base / BLOBS_SUBDIR
        self.sessions_path = self.db_dir / SESSIONS_JSON
        self.jobs_path = self.db_dir / JOBS_JSON
        self.sessions_blob_root = self.blobs_dir / SESSIONS_BLOB_DIR
        self.jobs_blob_root = self.blobs_dir / JOBS_BLOB_DIR
        self.db_dir.mkdir(parents=True, exist_ok=True)
        self.blobs_dir.mkdir(parents=True, exist_ok=True)
        self.sessions_blob_root.mkdir(parents=True, exist_ok=True)
        self.jobs_blob_root.mkdir(parents=True, exist_ok=True)

    def _lock_and_read_sessions(self) -> dict[str, Any]:
        lock_path = self.db_dir / DB_LOCK_NAME
    ...
    def session_blob_dir(self, session_id: str) -> Path:
        """Path to blobs/sessions/<session_id>/ (created on first use)."""
        p = self.sessions_blob_root / session_id
        p.mkdir(parents=True, exist_ok=True)
        return p

    def job_blob_dir(self, job_id: str) -> Path:
        """Path to blobs/jobs/<job_id>/ (created on first use)."""
        p = self.jobs_blob_root / job_id
        p.mkdir(parents=True, exist_ok=True)
        return p
